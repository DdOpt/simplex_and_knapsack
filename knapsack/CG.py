...
w = [1,2,2,3,4]
capacity = 6
#w = [np.random.randint(1,capacity-30) for i in range(capacity)]
A = np.eye(len(w))
b = np.ones(len(w))
try:
    #the rmp model
    masterpro = Model("rmp")
    y = masterpro.addMVar(len(w),obj=1,vtype=GRB.CONTINUOUS,name="y")
    ...
    while pricepro.objVal > 1:
        ...
        masterpro.addVar(obj=1.0,vtype=GRB.CONTINUOUS,name="new_patten_"+str(p),column=column)
        masterpro.optimize()
